...
// Conteúdo do email de boas-vindas por plano
const PLAN_EMAIL = {
  claro_plus: {
    nome: 'Claro+',
    badge: 'CLARO+ ATIVO',
...
// Constrói { subject, html } do email de boas-vindas adaptado ao plano.
// nome = primeiro nome do utilizador (ou null se desconhecido).
function buildWelcomeEmail(plano, nome) {
  const cfg = PLAN_EMAIL[plano] || null;
  const planoNome = cfg ? cfg.nome : 'Claro';
  const badge = cfg ? cfg.badge : 'SUBSCRIÇÃO ATIVA';
  const saudacaoNome = nome ? `, ${nome}` : '';
  const subject = `Bem-vindo ao ${planoNome}${saudacaoNome}!`;
  const ano = new Date().getFullYear();

  // Lista de features (omitida no fallback de plano desconhecido)
  const featuresBox = cfg ? `<div style="background:#F5F3EC;border-radius:12px;padding:24px;margin-bottom:24px;">
...
${cfg.features.map(f => `<tr><td style="padding:6px 0;font-size:13px;color:#3A3A38;"><span style="color:#1D9E75;font-weight:700;margin-right:8px;">✓</span>${f}</td></tr>`).join('\n')}
</table>
</div>` : '';
...
<div style="display:inline-block;background:rgba(29,158,117,0.15);color:#1D9E75;font-size:12px;padding:4px 16px;border-radius:20px;margin-top:10px;">${badge}</div>
</td></tr>
<tr><td style="padding:40px;">
<p style="font-size:22px;font-weight:600;color:#1A1A18;margin:0 0 12px;">Bem-vindo ao ${planoNome}${saudacaoNome}! 🎉</p>
<p style="font-size:15px;color:#5A5955;line-height:1.7;margin:0 0 24px;">A tua subscrição está ativa. Obrigado por confiares no Claro.</p>
${featuresBox}
...
<p style="font-size:12px;color:#A8A69F;margin:0;">© ${ano} Claro-app.ch · Montreux, Suíça</p>
</td></tr>
</table>
</td></tr>
</table>
...
module.exports = async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (email) {
      ...
      // Enviar email de boas-vindas via Brevo, adaptado ao plano
      const nomeReal = session.customer_details?.name || null;
      const welcome = buildWelcomeEmail(plano, nomeReal);
