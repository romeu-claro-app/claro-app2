...
// ALTER TABLE profiles ADD COLUMN IF NOT EXISTS notif_seguro_gratuito_enviada boolean DEFAULT false;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Buscar utilizadores gratuitos com fcm_token que ainda não receberam esta notificação
    const profilesRes = await fetch(
      `${SUPABASE_URL}/rest/v1/profiles?plano=eq.gratuito&fcm_token=not.is.null&notif_seguro_gratuito_enviada=eq.false&select=id,fcm_token`,
      {
        headers: {
        ...
        if (fcmRes.ok) {
          await fetch(
            `${SUPABASE_URL}/rest/v1/profiles?id=eq.${profile.id}`,
            {
              ...
              body: JSON.stringify({ notif_seguro_gratuito_enviada: true }),
            }
          );
          sent++;
        } else {
    ...
    return res.status(200).json({
      message: `Notificações sazonais seguro (gratuito) enviadas: ${sent}`,
      sent,
